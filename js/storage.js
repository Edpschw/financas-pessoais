...
function normalizePosition(pos) {
  ...
  // Detalhe por posição individual (nome/rentabilidade por período/taxa/vencimento),
  // quando a extração por produto do PDF bateu com o resumo — ver parsePortfolioDetail
  // em pdf-import.js. Preservado no snapshot pra não se perder a cada leitura.
  if (pos.maturity) out.maturity = pos.maturity;
  if (typeof pos.contractedRatePct === "number") out.contractedRatePct = pos.contractedRatePct;
  if (pos.returns) out.returns = pos.returns;
  if (pos.liquidity) out.liquidity = pos.liquidity;
  return out;
}
